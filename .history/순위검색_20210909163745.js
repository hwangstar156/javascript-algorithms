...
class Tree {
  ...
  insert(arr, idx) {
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      if (currentNode.child[currentStr] === undefined && i !== arr.length - 1) {
        currentNode.child[currentStr] = new Node(currentStr, idx);
      } else if (
        currentNode.child[currentStr] === undefined &&
        i === arr.length - 1
      ) {
        console.log(arr[i], currentNode.child[currentStr]);
      }
  ...
  search(array, idx, visit = {}, currentNode = this.root) {
    ...
    if (idx === 4) {
      if (array[idx] >= Number(Object.values(currentNode.child)[0].value)) {
        console.log(array, currentNode);
        result[currentNode.index] += 1;
      }
      return;
    }
    if (currentNode.child[array[idx]]) {
      this.search(array, idx + 1, visit, currentNode.child[array[idx]]);
    }
